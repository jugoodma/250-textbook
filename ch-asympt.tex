...
Informally, \(f(n) \in \Oh(g(n))\)just means that eventually for big enough \(n\) we have \(f(n) < g(n)\). For example, \(2^n \in \Oh(n!)\) since \(2^n < n!\) for \(n \geq 4\) (\(n \in \N\)). So the growth-rate of \(2^n\) is bounded \textit{above} by \(n!\). Similarly, we could say that \(n! \in \Omega(2^n)\) since \(n! > 2^n\) for \(n \geq 4\). So \(n!\) is bounded \textit{below} by \(2^n\). Oftentimes in algorithm analysis we aim to show an algorithm has exact run-time (\(\Theta\)), and we do this by showing it has both \(\Oh\) and \(\Omega\) run-time. Sometimes, though, this is very hard to do.
