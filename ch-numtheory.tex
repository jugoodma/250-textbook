...
\begin{thm}{The Principle of (weak) Mathematical Induction \label{pmi-1}\index{Weak Induction!Induction}}
	\begin{center}
		\begin{tabular}{rl}
			& \(P(0)\) \\
...
\begin{thm}{The Principle of (strong) Mathematical Induction \label{psi}\index{Strong Induction!Induction}}
	\begin{center}
		\begin{tabular}{rl}
...
\index{Structural Induction!Induction}
...
\begin{enumerate}
	...
	\item Show that \(\floor{x} = x \Leftrightarrow x \in \Z\) (the \(\Leftarrow\) direction is more difficult).
	...
	\item 
	\item Prove \(\big(\bigwedge_{i=1}^{n-1} (p_i \Rightarrow p_{i+1})\big) \Rightarrow \big( (\bigwedge_{i=1}^{n-1} p_i) \Rightarrow p_n \big)\)
	\item 
	\item 
	\item Find the lower bound for the amount of dollars we can make using strictly 3, 7, and 15 dollar bills. This should be done with constructive induction. Then, prove your result is correct using either weak or strong induction.
	\item 
\end{enumerate}
