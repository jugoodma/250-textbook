...
\newcommand{\blz}{\mathbin{\blacklozenge}}
\exproof{
	...
	For example, \(a \blz d = a\) (the first operand comes from the row, and the second operand comes from the column).
	
	Is \(S\) closed under \(\blz\)? Justify.
}{
	\(S\) is closed under \(\blz\) since every possible outcome (as listed in the table) of the \(\blz\) operation results in an element in \(S\).
}
...
\begin{enumerate}
	...
	\item
	Show that \(x^2 + y^3 = 1\) has no solution if \(y\) is odd.
	\item
	Show that if \(x^2+y^2\) is odd then \(xy\) is even.
	\item
	Show that if \(y\) is odd then \(x^2 + y^3 = 1\) has no solution.
	\item
	Show that if \(x^2(y+3)\) is even then \(x\) is even or \(y\) is odd.
	...
	\item Prove that \(t_n \leq n \log_b n\) where \[t_n = \begin{cases} 0, & n = 0 \\ 2t_{\floor{\frac{n}{2}}} + n - 1, & n > 0 \end{cases}\]
	\textit{You will need to select a base \(b \in \Z^+\) during the step that makes the induction easy}
