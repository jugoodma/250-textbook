...
\begin{defn}[Symmetric\index{Symmetric!Relation}]
	A relation \(R\) on a set \(A\) is symmetric if \[(\forall a,b \in A)[(a,b) \in R \Rightarrow (b,a) \in R]\]
\end{defn}

\begin{defn}[Transitive\index{Transitive!Relation}]
...
% todo : write an actual application of this

Here are some other cool definitions that probably will not be tested.

\begin{defn}[Antisymmetric\index{Antisymmetric!Relation}]
...
Typically we define functions on sets of numbers (we will restrict it to subsets of \(\R\)). We use the following notation to denote a function \(f\): \[f(x) = \text{some rules}\] and we pair the input/output sets as follows: \[f : D \mapsto R\]
...
This is a short section, however it still deserves attention. % todo : fix 
