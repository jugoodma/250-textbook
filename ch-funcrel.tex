...
Basically, functions allow us to define a \textit{mapping} between sets. Typically we define functions on sets of numbers (we will restrict it to subsets of \(\R\)). We use the following notation to denote a function \(f\): \[f(x) = \text{some rules}\] and we pair the input/output sets as follows: \[f : D \mapsto R\]

\begin{defn}[Domain\index{Domain!Function}]
	The set of inputs to a function
\end{defn}

\begin{defn}[Range\index{Range!Function}]
	The set containing all outputs to a function. This may also be called the \textit{co-domain}
\end{defn}

\begin{defn}[Image\index{Image!Function}]
...
\begin{defn}[Injection (One-to-One)]
...
\begin{defn}[General Form\index{General Form!Sequence}]
...
\begin{defn}[Recursive Form\index{Recursive Form!Sequence}]
...
\begin{defn}[Closed Form\index{Closed Form!Sequence}]
...
\begin{example}
	The \textit{equality} relation \(\{(a,b) \mid a = b\} \subseteq \R \times \R\)
\end{example}
...
\begin{itemize}
	\item Relation - (Binary Relation) if $A$ and $B$ are sets, then a Relation from $A$ to $B$ is any subset of $A \times B$. $aRb$ means $(a,b) \in R$ where $a \in A$ and $b \in B$ and $R \subseteq A \times B$. A Relation over a set $A$ is simply a Relation from $A$ to $A$.
	\item Reflexive - a Relation $R$ on a set $A$ is Reflexive if\\$(\forall a \in A)[(a,a) \in R]$.
	\item Symmetric - a Relation $R$ on a set $A$ is Symmetric if\\$(\forall a,b \in A)[(a,b) \in R \Rightarrow (b,a) \in R]$.
	\item Antisymmetric - a Relation $R$ on a set $A$ is Antisymmetric if\\$(\forall a,b \in A)[(a,b) \in R \land (b,a) \in R \Rightarrow a = b]$.
	\item Transitive - a Relation $R$ on a set $A$ is Transitive if\\$(\forall a,b,c \in A)[(a,b) \in R \land (b,c) \in R \Rightarrow (a,c) \in R]$.
	\item Composite - (just something cool to note) if $R$ is a Relation from sets $A$ to $B$ and $S$ is a Relation from $B$ to $C$, then the Composite of $R$ and $S$, denoted $S \circ R$, is $\{(a,c) \mid a \in A \land c \in C \land (b \in B \Rightarrow (a,b) \in R \land (b,c) \in S)\}$
	\item Powers - (just something cool to note) if $R \subseteq A \times A$, then for $n \in \N^{\geq 1}, R^n = R^{n-1} \circ R$ where $R^1 = R$.
	\subitem $R \subseteq A \times A$ is Transitive iff $R^n \subseteq R$ (this is a theorem from your textbook)
	\item Equivalence Relation - a Relation $R$ on a set $A$ is an Equivalence Relation if it is Reflexive, Symmetric, and Transitive.
	\\\end{itemize}

\begin{defn}[Reflexive\index{Reflexive!Relation}]
...
\begin{defn}[Equivalence Relation\index{Equivalence Relation!Relation}]
